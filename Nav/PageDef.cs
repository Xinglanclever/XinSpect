...
public sealed class PageDef
{
    ...
    /// </summary>
    /// <remarks>
    /// 判準是「一般使用者看得懂並且用得到嗎」，不是「這頁重不重要」——直讀 MSR、編程效能計數器、
    /// 解 PCI 設定空間那些頁面對懂的人價值最高，但對只想知道「我的電腦是什麼配備、健不健康」的人
    /// 只是雜訊。簡易模式只是<b>不顯示</b>，不是停用：命令面板（Ctrl+K）照樣搜得到，
    /// 程式化跳頁也照樣到得了，所以不會有「功能不見了」這種事。
    /// </remarks>
